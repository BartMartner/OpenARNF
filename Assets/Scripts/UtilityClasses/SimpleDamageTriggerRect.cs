...
public class SimpleDamageTriggerRect : MonoBehaviour
{
    ...
    {
        if (!enabled) return; //this should happen automatically?
        _previousSetting = Physics2D.queriesHitTriggers;
        Physics2D.queriesHitTriggers = true;
        var o = transform.TransformDirection(offset);
        if (Physics2D.OverlapBoxNonAlloc(transform.position + o, size, 0, _collider2Ds, mask) > 0)
        {
    ...
    {
        var o = transform.TransformDirection(offset);
        Extensions.DrawSquare(new Rect(transform.position + o - (Vector3)size * 0.5f, size), Color.red);
    }
}
