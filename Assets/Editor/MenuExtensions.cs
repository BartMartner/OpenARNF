...
public class MenuExtensions : MonoBehaviour
{
    ...
    [MenuItem("Extensions/Tilemaps/Tilemap Pixel Snap Off")]
    static void TilemapPixelSnapOff()
    {
        bool dirty = false;
        var tilemaps = Resources.FindObjectsOfTypeAll<STETilemap>();
        foreach (var tilemap in tilemaps)
        {
            if (tilemap.PixelSnap)
            {
                dirty = true;
                tilemap.PixelSnap = false;
            }
        }
    ...
    [MenuItem("Extensions/Tilemaps/Assign Environment Material %&n")]
    static void AssignEnvMaterial()
    {
        bool dirty = false;
        var tilemaps = Resources.FindObjectsOfTypeAll<STETilemap>();

        var generic = AssetDatabase.LoadAssetAtPath<Material>("Assets/Materials/StencilMask.mat");

        var surfaceMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/Surface.mat");
        var caveMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/Caves.mat");
        var factoryMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/Factory.mat");
        var coolantSewersMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/CoolantSewers.mat");
        var buriedCityMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/BuriedCity.mat");
        var glitchMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/Glitch.mat");
        var forestSlumsMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/ForestSlums.mat");
        var minesMaterial = AssetDatabase.LoadAssetAtPath<Material>("Assets/Resources/Materials/CrystalMines.mat");
        ...
        foreach (var tilemap in tilemaps)
        {
            ...
            switch (tilemap.Tileset.name)
            {
                case "Surface":
                    if (tilemap.Material != surfaceMaterial)
                    {
                        tilemap.Material = surfaceMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change surface material on " + tilemap.name);
                    }
                    break;
                case "Caves":
                    if (tilemap.Material != caveMaterial)
                    {
                        tilemap.Material = caveMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change cave material on " + tilemap.name);
                    }
                    break;
                case "Factory":
                    if (tilemap.Material != factoryMaterial)
                    {
                        tilemap.Material = factoryMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change factory material on " + tilemap.name);
                    }
                    break;
                case "CoolantSewers":
                    if (tilemap.Material != coolantSewersMaterial)
                    {
                        tilemap.Material = coolantSewersMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change coolantSewers material on " + tilemap.name);
                    }
                    break;
                case "BuriedCity":
                    if (tilemap.Material != buriedCityMaterial)
                    {
                        tilemap.Material = buriedCityMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change buriedCity material on " + tilemap.name);
                    }
                    break;
                case "GreyBox":
                case "BeastGuts":
                    if (tilemap.Material != generic)
                    {
                        tilemap.Material = generic;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change stencil material on " + tilemap.name);
                    }
                    break;
                case "Glitch":
                    if (tilemap.Material != glitchMaterial)
                    {
                        tilemap.Material = glitchMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        Debug.Log("Change glitchMaterial material on " + tilemap.name);
                    }

                    DestroyImmediate(tilemap.gameObject.GetComponent<PaletteCycling>());

                    var cycle = tilemap.gameObject.GetComponent<TilemapPaletteCycling>();
                    if (!cycle)
                    {
                        cycle = tilemap.gameObject.AddComponent<TilemapPaletteCycling>();
                    }
                    cycle.paletteCycle = AssetDatabase.LoadAssetAtPath<PaletteCycle>("Assets/Resources/PaletteCycles/GlitchCycle.asset");
                    cycle.cycleFrequency = 0.3846154f;
                    dirty = true;

                    break;
                case "ForestSlums":
                    if (tilemap.Material != forestSlumsMaterial)
                    {
                        tilemap.Material = forestSlumsMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change forestSlumsMaterial material on " + tilemap.name);
                    }
                    break;
                case "CrystalMines":
                    if (tilemap.Material != minesMaterial)
                    {
                        tilemap.Material = minesMaterial;
                        tilemap.PixelSnap = false;
                        tilemap.Refresh();
                        dirty = true;
                        Debug.Log("Change minesMaterial material on " + tilemap.name);
                    }
                    break;
                case "CrystalCave":
                    if (tilemap.name == "Crystal" && tilemap.SortingLayerName != "AboveFadeAways")
                    {
                        tilemap.SortingLayerName = "AboveFadeAways";
                        tilemap.OrderInLayer = -1;
                        dirty = true;
                        Debug.Log("Changed sorting layer on " + tilemap.name);
                    }
                    break;
            }
        }
